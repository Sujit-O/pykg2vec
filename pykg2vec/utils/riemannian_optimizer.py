...
class RiemannianOptimizer(Optimizer):
    ...
    def step(self, lr=None):
        ...
        for group in self.param_groups:
            for i, p in enumerate(group["params"]):
                ...
                if lr is None:
                    lr = group["lr"]
                if self.param_names[i] in ["ent_embedding.weight", "rel_embedding.weight"]:
                    d_p = self.poincare_grad(p, d_p)
                    p.data = self.poincare_update(p, d_p, lr)
                else:
                    p.data = self.euclidean_update(p, d_p, lr)
        return loss
    ...
    def poincare_update(p, d_p, lr):
        v = -lr * d_p
        p.data = RiemannianOptimizer.full_p_exp_map(p.data, v)
        return p.data
    ...
    def full_p_exp_map(x, v):
        ...
        return RiemannianOptimizer.p_sum(x, y)
