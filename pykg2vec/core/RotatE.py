...
class RotatE(ModelMeta):
    ...
    def test_step(self):
        ...
        tail_pos_score = self.comp_mul_and_min(h_vec_r, h_vec_i, \
            r_vec_r, r_vec_i, self.ent_embeddings_real, self.ent_embeddings_imag)

        _, self.head_rank      = tf.nn.top_k(head_pos_score, k=num_entity)
        _, self.tail_rank      = tf.nn.top_k(tail_pos_score, k=num_entity)
        _, self.norm_head_rank = tf.nn.top_k(head_pos_score, k=num_entity)
        _, self.norm_tail_rank = tf.nn.top_k(tail_pos_score, k=num_entity)
