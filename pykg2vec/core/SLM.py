...
class SLM(ModelMeta):
    ...
    def test_step(self):
        ...
        energy_h = tf.reduce_sum(r_vec*self.layer(self.ent_embeddings, t_vec), -1)
        energy_t = tf.reduce_sum(r_vec*self.layer(h_vec, self.ent_embeddings), -1)

        self.ent_embeddings = tf.nn.l2_normalize(self.ent_embeddings, axis=1)
        self.rel_embeddings = tf.nn.l2_normalize(self.rel_embeddings, axis=1)

        norm_h_vec, norm_r_vec, norm_t_vec = self.embed(self.test_h, self.test_r, self.test_t)
        norm_energy_h = tf.reduce_sum(norm_r_vec*self.layer(self.ent_embeddings, norm_t_vec), -1)
        norm_energy_t = tf.reduce_sum(norm_r_vec*self.layer(norm_h_vec, self.ent_embeddings), -1)
